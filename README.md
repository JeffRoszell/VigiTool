...
CVT files carry each measure three ways — whole block, central location, peripheral locations — in `performance_by_location`, and per period in `period_performance[].by_location`. For sessions recorded before that block existed, `python tools/cvt_location_report.py` rebuilds the same split from the saved trials into one CSV:

```bash
python tools/cvt_location_report.py                # everything under data/
python tools/cvt_location_report.py data/P001 --by-period
```
