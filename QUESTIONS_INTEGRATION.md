...
19. **Calibration timing in the session**: Eye-tracker calibration (Tobii 9-point) and B-Alert impedance checks happen through iMotions' own UI before recording starts — PsychoPy can't trigger them mid-session. Confirm the intended session order:
    ...
    > **Resolved (June 2026, PI)**: Session order confirmed (Smarteye calibration, not Tobii). **Yes — every 5-minute break requires eye-tracking recalibration.** (Originally "between the two 24-minute blocks of each task, and between CVT and PVT"; **superseded Sept 2026** — the PVT is now a single 10-minute block, so the only breaks are between the two CVT blocks and between tasks. Two recalibration holds per session, not three.) Implemented as `session_utils.recalibration_hold`: after each timed break the app holds on an RA screen until recalibration is confirmed, bracketed by `recalibration_start`/`recalibration_end` markers.
